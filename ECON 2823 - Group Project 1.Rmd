...
## NOTE: this tiebreaker is currently only breaking ties based on goal differential, it needs to then revert to breaking ties by "pointsfor", and then with a random, uniform draw.
...
rankplot <- ggplot(aa, aes(x=reorder(team, -mean), y=mean))+geom_bar(stat='identity', fill="#4eb14e") +  xlab("Team Names") + ylab("Expected Rank") + ggtitle("Average Team Rankings Over 10,000 Simulations") + coord_flip()
```
...
```{r message=FALSE}
check <- select(cc, "team", "oldrank", "oldearnings","newrank", "newearnings", "diffearn")
earnplot
```
